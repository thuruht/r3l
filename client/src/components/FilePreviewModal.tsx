...
const FilePreviewModal: React.FC<FilePreviewModalProps> = ({
  ...
  // Reset resolved values when the file or props change
  useEffect(() => {
    setResolvedFilename(filename);
    setResolvedMimeType(mimeType);
  }, [fileId, filename, mimeType]);

  // Post to Drift History on load (runs after metadata resolves filename/mimeType)
  useEffect(() => {
    if (fileId && resolvedFilename && currentUser && visibility !== 'me') {
      fetch('/api/history', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ file_id: fileId, filename: resolvedFilename, mime_type: resolvedMimeType }),
      }).catch((err) => console.error('Failed to post drift history', err));
    }
  }, [fileId, resolvedFilename, resolvedMimeType, currentUser, visibility]);
  ...
  const isImage = resolvedMimeType.startsWith('image/') || !!resolvedFilename.match(/\.(png|jpg|jpeg|gif|bmp|svg|webp|ico|tiff?|avif|heic|heif)$/i);
  const isAudio = resolvedMimeType.startsWith('audio/') || !!resolvedFilename.match(/\.(mp3|wav|ogg|flac|aac|m4a|wma|opus)$/i);
  const isVideo = resolvedMimeType.startsWith('video/') || !!resolvedFilename.match(/\.(mp4|webm|avi|mov|wmv|mkv|flv|m4v|3gp)$/i);
  const isPDF = resolvedMimeType === 'application/pdf' || !!resolvedFilename.match(/\.pdf$/i);

  const isSpreadsheet = !!(resolvedFilename.match(/\.(xlsx|xls|csv)$/i) || resolvedMimeType.match(/excel|spreadsheet|csv/));
  const isDocx = !!(resolvedFilename.match(/\.(docx)$/i) || resolvedMimeType.match(/wordprocessingml/));
  const is3D = !!(resolvedFilename.match(/\.(gltf|glb|stl|obj|fbx|ply|3mf)$/i) || resolvedMimeType.match(/gltf|model\/stl|model\/obj|model\/fbx/));
  const isUSDZ = !!resolvedFilename.match(/\.usdz$/i) || resolvedMimeType === 'model/vnd.usdz+zip';
  const isRichText = !!resolvedFilename.match(/\.(notes|tiptap)$/i);
  const isMarkdown = !!resolvedFilename.match(/\.(md|markdown|qmd)$/i);
  const isNotebook = !!resolvedFilename.match(/\.ipynb$/i) || resolvedMimeType === 'application/x-ipynb+json';
  const isEpub = !!resolvedFilename.match(/\.(epub)$/i) || resolvedMimeType === 'application/epub+zip';
  const isMermaid = !!resolvedFilename.match(/\.(mmd|mermaid)$/i);
  const isExcalidraw = !!resolvedFilename.match(/\.excalidraw$/i) || resolvedMimeType === 'application/vnd.excalidraw+json';
  const isParquet = !!resolvedFilename.match(/\.parquet$/i) || resolvedMimeType === 'application/vnd.apache.parquet';
  const isQML = !!resolvedFilename.match(/\.qml$/i);

  const isZip = !isEpub && !isSpreadsheet && !isDocx && !isNotebook && !!resolvedMimeType.match(/zip|compressed|archive/);
  const isText =
    !isRichText &&
    !isMarkdown &&
    !isSpreadsheet &&
    ...
    (resolvedMimeType.startsWith('text/') ||
      !!resolvedMimeType.match(/json|xml|javascript|typescript|python|x-sh/) ||
      !!resolvedFilename.match(
        /\.(txt|md|qmd|json|xml|html|css|js|jsx|ts|tsx|py|java|c|cpp|h|rs|go|rb|php|sh|bash|sql|yaml|yml|toml|ini|conf|log|mmd|mermaid)$/i,
      ));
  const isHTML = resolvedMimeType === 'text/html' || resolvedFilename.endsWith('.html');
  const isWebCode = isHTML || !!resolvedFilename.match(/\.(html|css|js|jsx|ts|tsx)$/);
  ...
  useEffect(() => {
    ...
    const fetchFile = async () => {
      ...
      try {
        ...
        // Resolve filename/mimeType from metadata when not provided by parent
        const ef = filename || meta.filename || '';
        const em = mimeType || meta.mime_type || '';
        setResolvedFilename(ef);
        setResolvedMimeType(em);
        ...
        // Compute type flags from effective values (state updates are async, use local vars here)
        const efIsRichText = !!ef.match(/\.(notes|tiptap)$/i);
        const efIsMarkdown = !!ef.match(/\.(md|markdown|qmd)$/i);
        const efIsQML = !!ef.match(/\.qml$/i);
        const efIsSpreadsheet = !!(ef.match(/\.(xlsx|xls|csv)$/i) || em.match(/excel|spreadsheet|csv/));
        const efIsDocx = !!(ef.match(/\.(docx)$/i) || em.match(/wordprocessingml/));
        const efIsNotebook = !!ef.match(/\.ipynb$/i) || em === 'application/x-ipynb+json';
        const efIsText =
          !efIsRichText && !efIsMarkdown && !efIsSpreadsheet && !efIsDocx && !efIsNotebook && !efIsQML &&
          (em.startsWith('text/') ||
            !!em.match(/json|xml|javascript|typescript|python|x-sh/) ||
            !!ef.match(/\.(txt|md|qmd|json|xml|html|css|js|jsx|ts|tsx|py|java|c|cpp|h|rs|go|rb|php|sh|bash|sql|yaml|yml|toml|ini|conf|log|mmd|mermaid)$/i));

        if (efIsText || efIsRichText || efIsMarkdown || efIsQML) {
  ...
  const handleRemixSave = async () => {
    try {
      const blob = new Blob([editContent], { type: resolvedMimeType });
      const formData = new FormData();
      formData.append('file', blob, `remix-${resolvedFilename}`);
    ...
    <>
      ...
      <button
        ...
        style={{
          background: 'transparent',
          border: 'none',
          color: boosted ? 'var(--accent-sym)' : 'var(--text-primary)',
          display: 'flex',
          alignItems: 'center',
          gap: 'var(--spacing-xs)',
          flexShrink: 0,
      ...
      <BookmarkButton fileId={fileId || ''} isMobile={isMobile} />
      <button
        onClick={onDownload}
        title="Download"
        aria-label="Download"
        ...
        <>
          <button
            ...
            style={{
              ...
              width: isMobile ? '100%' : 'auto',
              padding: isMobile ? 'var(--spacing-sm)' : '0',
              opacity: isText && content === null ? 0.5 : 1,
            }}
          >
          ...
          <button
            ...
            aria-label="Edit"
            disabled={loading || (isText && content === null)}
            style={{
              background: 'transparent',
              border: 'none',
              color: 'var(--text-primary)',
              ...
              width: isMobile ? '100%' : 'auto',
              padding: isMobile ? 'var(--spacing-sm)' : '0',
              opacity: loading || (isText && content === null) ? 0.5 : 1,
            }}
          >
      ...
      {isWebCode && !isEditing && (
        <button
          ...
          style={{
            background: 'transparent',
            border: 'none',
            color: showPreview ? 'var(--accent-sym)' : 'var(--text-primary)',
            flexShrink: 0,
            display: 'flex',
            alignItems: 'center',
            gap: 'var(--spacing-xs)',
          ...
          {!collabEnabled ? (
            ...
            <div
              style={{
                ...
                alignItems: 'center',
                gap: 'var(--spacing-xs)',
                color: collabStatus === 'connected' ? 'var(--accent-sym)' : 'var(--text-secondary)',
                flexShrink: 0,
                padding: isMobile ? 'var(--spacing-sm)' : '0',
              }}
            >
      ...
      {!isEditing && (
        <>
          <button
            onClick={() => (isText || isRichText ? setIsEditing(true) : setShowRemixUpload(true))}
            title="Remix"
            aria-label="Remix"
            style={{
              background: 'transparent',
          ...
          <button
            ...
            title="Vote to Archive"
            aria-label="Vote to Archive"
            style={{
              background: 'transparent',
              border: 'none',
              color: 'var(--text-primary)',
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {!isMobile && <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>{actionButtons}</div>}
                ...
                <button
                  ...
                  style={{ background: 'transparent', border: 'none', color: 'var(--text-primary)', flexShrink: 0, marginLeft: '5px' }}
                >
            ...
            >
              ...
              {!loading && !error && (
                <>
                  {remixParent && (
                    <div
                      style={{
                        ...
                        borderRadius: '4px',
                        borderLeft: '2px solid var(--accent-sym)',
                        fontSize: '0.82rem',
                        color: 'var(--text-secondary)',
                      }}
                    >
                  ...
                  {isAudio && <AudioWaveform src={`/api/files/${fileId}/content`} mimeType={resolvedMimeType} />}
                    ...
                    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div style={{ width: '100%', maxWidth: '900px' }}>
                        <Plyr
                          source={{
                            type: 'video',
                            sources: [{ src: `/api/files/${fileId}/content`, type: resolvedMimeType }],
                          }}
                          options={{
                            controls: [
                              'play-large',
                  ...
                  {isUSDZ && <USDZViewer url={`/api/files/${fileId}/content`} filename={resolvedFilename} />}
                  ...
                  {!isImage &&
                    ...
                    !isEpub && (
                      <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>
                        ...
                        <div style={{ fontSize: '0.8rem', marginTop: '10px' }}>{resolvedMimeType}</div>
                      </div>
                    )}
                  ...
                  {isText && !isEditing && !showPreview && (
                    <pre style={{ whiteSpace: 'pre-wrap', fontFamily: 'monospace', color: 'var(--text-primary)' }}>{content}</pre>
                  )}
                  ...
                  {ownerId && currentUser && fileId && <CommentsSection fileId={fileId} currentUser={currentUser} fileOwnerId={ownerId} />}
                </>
              )}
            </div>
          ...
          {showRemixUpload && (
            <div style={{ pointerEvents: 'auto' }} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
              <UploadModal
                ...
                onUploadComplete={() => {
                  showToast('Remix uploaded', 'success');
                  setShowRemixUpload(false);
                }}
                parentId={fileId}
              />
            </div>
