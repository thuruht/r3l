    ...
    <>
      <div style={{ position: 'relative', marginRight: '10px', zIndex: 'var(--z-dropdown)' }}>
        <div style={{
          ...
          <TablerIcons.IconSearch size={ICON_SIZES.md} color={isFocused ? 'var(--accent-sym)' : 'var(--text-secondary)'} />
          <input
            type="text"
            placeholder="Search..."
          ...
          />
          {loading && <div className="icon-spin" style={{ marginRight: '5px' }}><TablerIcons.IconLoader size={ICON_SIZES.sm} color="var(--text-secondary)" /></div>}
          {query && !loading && (
             <button
                onClick={closeSearch}
             ...
             >
                <TablerIcons.IconX size={ICON_SIZES.sm}/>
             </button>
          )}
        </div>
      </div>
        ...
        <div className="search-modal-overlay fade-in" style={{
          ...
          <div className="glass-panel" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>
                ...
                <button onClick={closeSearch} aria-label="Close search results" style={{ background: 'none', border: 'none', color: 'var(--text-secondary)' }}><TablerIcons.IconX size={ICON_SIZES.xl} /></button>
            </div>
            ...
            {!loading && results.length === 0 && (
                <div style={{ padding: '30px', textAlign: 'center', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
                    <TablerIcons.IconBroadcast size={ICON_SIZES['2xl']} stroke={1} style={{ opacity: 0.7 }} />
...
export const RandomUserButton: React.FC<DiscoveryProps> = ({ onNavigate }) => {
  ...
  return (
    ...
    >
      <TablerIcons.IconDice size={ICON_SIZES.lg} />
    </button>
  );
};
