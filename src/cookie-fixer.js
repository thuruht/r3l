...
class OrcidCookieFixer {
  ...
  fix() {
    ...
    const orcidCallbackRegex = /\/\/ Complete ORCID auth[\s\S]+?\/api\/auth\/orcid\/callback[\s\S]+?async function/m;
    ...
    const cookieSettingRegex = /\/\/ For direct browser requests, set cookies and redirect[\s\S]+?headers\.append\('Set-Cookie'.+?;/m;
    ...
    const cookieSectionIndex = startIndex + this.routerContent.substring(startIndex).indexOf(cookieSection);
    ...
    const updatedContent = this.routerContent.substring(0, cookieSectionIndex) + 
                          newCookieSection + 
  ...
  fixGitHubCallback(content) {
    ...
    const githubCallbackRegex = /\/\/ Complete GitHub auth[\s\S]+?\/api\/auth\/github\/callback[\s\S]+?async function/m;
    ...
    // Apply the replacement
    const updatedContent = content.substring(0, cookieSectionIndex) + 
                          newCookieSection + 
  ...
  fixAuthStateCookieMethod(content) {
    ...
    const methodRegex = /private fixAuthStateCookie\(request: Request\): Response {[\s\S]+?return new Response\([\s\S]+?}\);[\s\S]+?}/m;
    ...
    // Apply the replacement
    const updatedContent = content.substring(0, methodIndex) + 
                          newMethod + 
