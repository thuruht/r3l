...
export class ContentCopyHandler {
  ...
  async handleDownloadContent(request: AuthenticatedRequest, env: Env): Promise<Response> {
    try {
      ...
      if (!await contentHandler.canAccessContent(contentId, request.authenticated ? request.userId : null, env)) {
  ...
  private async recordArchiveVote(
    ...
    if (existingVote && voteType !== 'explicit') {
      const currentVotes = await env.R3L_DB.prepare(`
        ...
        WHERE id = ?
      `).bind(contentId).first<{ archive_votes: number }>();
    ...
    await env.R3L_DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).bind(crypto.randomUUID(), contentId, userId, voteType, Date.now()).run();
   ...
   */
  private async getUserDailyVotes(userId: string, env: Env): Promise<{
