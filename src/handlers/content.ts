...
export class ContentHandler {
  ...
  async createContent(
    ...
    const expiresAt = isEphemeral ? 
      (data.expiresIn === 0 ? null : now + (data.expiresIn || 7) * 24 * 60 * 60 * 1000) : 
      null;
    ...
    if (data.location) {
      if (Sanitizer.validateCoordinates(data.location.lat, data.location.lng)) {
        ...
        this.logger.warn('Invalid coordinates provided for content', { contentId, location: data.location });
      }
    }
  ...
  async updateContent(
    ...
    if (updates.isEphemeral !== undefined) {
      ...
      if (!updates.isEphemeral) {
        ...
        const expiresAt = updates.expiresIn === 0 ? null : 
          Date.now() + updates.expiresIn * 24 * 60 * 60 * 1000;
    ...
    if (content.user_id !== userId) {
      ...
      if (content.file_key) {
        try {
          ...
          this.logger.error('Failed to delete content file', error as Error, { contentId, fileKey: content.file_key });
        }
  ...
  async voteForCommunityArchive(
    ...
    const voteResult = await env.R3L_DB.prepare(`
      ...
      WHERE content_id = ?
    `).bind(contentId).first<{vote_count: number}>();
  ...
  async getRandomContent(env: Env): Promise<ContentItem | null> {
    ...
    const content = await env.R3L_DB.prepare(`
      ...
      ORDER BY RANDOM()
      LIMIT 1
    `).bind(Date.now()).first<ContentItem>();
