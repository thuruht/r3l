...
export class DrawerHandler {
  ...
  async createDrawer(
    ...
    `
      ...
      .bind(drawerId, userId, name, description, isPublic ? 1 : 0, now, now)
      .run();
      ...
      return {
        ...
        recentFiles: (filesResult.results || []).map(file => ({
          ...
          expiresIn: file.expires_at
            ? Math.ceil((file.expires_at - Date.now()) / (1000 * 60 * 60 * 24))
            : undefined,
        })),
      };
    } catch (error) {
  ...
  async addContentToDrawer(
    ...
    `
      ...
      .bind(drawerContentId, drawerId, contentId, Date.now(), note || '')
      .run();
  ...
  async getDrawerContents(
    drawerId: string,
    env: Env
  ): Promise<(DrawerContentItem & ContentDetails)[]> {
    const drawer = await this.getDrawer(drawerId, env);
    if (!drawer) {
      throw new Error('Drawer not found');
    }
    ...
    `
      ...
      .bind(drawerId)
      .all<DrawerContentItem & ContentDetails>();
  ...
  async updateDrawerContentNote(
    ...
    `
      ...
      .bind(drawerContentId)
      .first<DrawerContentItem & { drawer_owner_id: string }>();
    ...
    `
      ...
      .bind(Date.now(), drawerContent.drawer_id)
      .run();
  }
