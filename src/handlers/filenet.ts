...
export class FilenetHandler {
  ...
  async handleFileUpload(request: AuthenticatedRequest, env: Env): Promise<Response> {
    try {
      ...
      return new Response(`File upload failed: ${error?.message || 'Unknown error'}`, { status: 500 });
    }
  }
  ...
  async getFile(request: AuthenticatedRequest, env: Env): Promise<Response> {
    try {
      ...
      `).bind(fileId).first<ContentRecord>();
      ...
      if (request.authenticated && request.userId && isDownload) {
        ...
        contentHandler.recordDownload(fileId, request.userId, env)
      ...
      return new Response(`File retrieval failed: ${error?.message || 'Unknown error'}`, { status: 500 });
    }
  }
  ...
  async listUserFiles(request: AuthenticatedRequest, env: Env): Promise<Response> {
    try {
      ...
      const files = await env.R3L_DB.prepare(sql).bind(...params).all<ContentRecord>();
      ...
      return new Response(`File listing failed: ${error?.message || 'Unknown error'}`, { status: 500 });
    }
  }
}
