...
// Create memory caches with appropriate TTLs
const dataPointsCache = new MemoryCache<string, GeoPoint[]>(5 * 60 * 1000); // 5 minutes
const pointCache = new MemoryCache<string, GeoPoint | null>(5 * 60 * 1000); // 5 minutes
...
export class GlobeHandler {
  ...
  constructor() {
    ...
    const dataPointsProcessor: BatchProcessor<DataPointRequest, GeoPoint[]> = 
      async (requests, env) => {
        ...
        for (const [userId, groupRequests] of userGroups.entries()) {
          ...
          for (let i = 0; i < requests.length; i++) {
            ...
            if (request.userId === actualUserId) {
              ...
              if (request.bbox) {
                results[i] = allPoints.filter(point => 
    ...
    const pointProcessor: BatchProcessor<PointRequest, GeoPoint | null> =
      async (requests, env) => {
  ...
  async getDataPoints(
    ...
    // Check cache first
    const cachedResult = dataPointsCache.get(cacheKey);
    ...
    // Store in cache
    dataPointsCache.set(cacheKey, result);
  ...
  async getPointById(
    ...
    // Check cache first
    const cachedResult = pointCache.get(cacheKey);
    if (cachedResult !== undefined) {
      return cachedResult;
    }
   ...
   */
  private async batchFetchPointsByIds(
    pointIds: string[],
    env: Env
  ): Promise<(GeoPoint | null)[]> {
    ...
    try {
      ...
      const result = await env.R3L_DB.prepare(query).bind(...pointIds).all();
  ...
  async updateDataPoint(
    ...
    try {
      ...
      // Invalidate caches
      dataPointsCache.clear();
      pointCache.delete(`point:${pointId}:null`);
