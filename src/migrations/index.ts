    ...
    for (const migration of migrationsToRun) {
      try {
        ...
        if (direction === 'up' && !applied) {
          ...
          this.logger.debug(`Skipping migration: ${migration.name} (already ${direction === 'up' ? 'applied' : 'rolled back'})`);
        }
      } catch (error) {
        this.logger.error(`Error running migration ${migration.id}: ${migration.name}`, error as Error);
