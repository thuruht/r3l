...
export class MessagingHandler {
   ...
   * @param attachments Optional array of attachment IDs (R2 object keys)
  ...
  async sendMessage(
    ...
    const now = Date.now();
    const isEncrypted = false; // Default value, can be set by parameter if needed

    // Store message in database
    await env.R3L_DB.prepare(
      `
      INSERT INTO direct_messages (
        id, from_user_id, to_user_id, content, attachments, is_encrypted, is_read, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `
    )
      .bind(
    ...
    `
      ...
      .bind(messageId, now, fromUserId, toUserId, toUserId, fromUserId)
      .run();
    ...
    try {
      ...
      const connectionsObj = env.R3L_CONNECTIONS.get(env.R3L_CONNECTIONS.idFromName(toUserId));
  ...
  private async ensureConversationExists(
    ...
    if (!existingConversation) {
      ...
      `
        ...
        .bind(conversationId, first, second, now, now)
        .run();
    }
  }
  ...
  async getMessagesBetweenUsers(
    ...
    let query = `
      SELECT 
        id, from_user_id, to_user_id, content, attachments, 
        is_encrypted, is_read, created_at
  ...
  async getUserConversations(
    ...
    const conversationsWithCounts = await Promise.all(
      ...
      `
        ...
        const hasAttachments = conv.last_message_attachments !== null;
   ...
   * @param env Environment
   */
  async markMessagesAsRead(userId: string, fromUserId: string, env: Env): Promise<void> {
    await env.R3L_DB.prepare(
      `
   ...
   * @param env Environment
   */
  async markConversationAsRead(userId: string, conversationId: string, env: Env): Promise<void> {
    ...
    `
      ...
      .bind(conversationId)
      .first<{ user_id_a: string; user_id_b: string }>();
   ...
   * @param env Environment
   * @returns Conversation data including other user details and messages
   */
